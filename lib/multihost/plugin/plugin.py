...
class MultihostPlugin(object):
    ...
    def pytest_make_collect_report(self, collector: pytest.Collector) -> pytest.CollectReport:
        """
        If multiple topology marks are present on the collected test, we
        parametrize need to parametrize it. In order to do so, the test is
        replaced with multiple clones, one for each topology.
...
def pytest_configure(config: pytest.Config):
    ...
    config.addinivalue_line(
        'markers', 'topology(name: str, topology: lib.multihost.topology.Topology, /, *, fixture1=target1, ...): '
        + 'topology required to run the test'
    )
